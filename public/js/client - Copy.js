...
function setupMeasurers() {
	var wsOnMessageTable = document.getElementById('ws-onmessage');
	window.getMeasurer('ws.onmessage').addListener(function(log, measurer) {
		var row = wsOnMessageTable.insertRow(0);
		row.insertCell(0).innerHTML = 'ws.onmessage';
		row.insertCell(1).innerHTML = log.endpoint;
		row.insertCell(2).innerHTML = log.size;
		row.insertCell(3).innerHTML = log.data;
		row.insertCell(4).innerHTML = log.time;
		
		var len = wsOnMessageTable.rows.length
		if (len > 100) {
			wsOnMessageTable.deleteRow(len - 1);
		}
	});
}